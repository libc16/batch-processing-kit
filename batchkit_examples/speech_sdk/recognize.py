...
class FileRecognizer:
    ...
    def __recognize_with_retries(self, audio_file, json_data, cancellation_token) -> float:
        ...
        while retry_count < max_retries:
            try:
                ...
                return result
            except UserTimeoutReachedException as e:
                retry_count += 1
    ...
    def __recognize(self, audio_file, json_data, cancellation_token) -> float:
        ...
        useUserSetTimeout = self._recognize_timeout > 0
        userTimeoutReached = False

        self._log_event_queue.log(LogLevel.INFO, f"Recognize timeout is set? {useUserSetTimeout}. Remaining timeout: {remaining_recognize_timeout} seconds.")
        ...
        while True:
            if useUserSetTimeout and not done_event.is_set() and not cancellation_token.is_set() and time.time() - start_time >= remaining_recognize_timeout:
                # If we are here, it means that the Speech SDK has not made any forward progress
                # for a long time, and we need to stop waiting.
                raise UserTimeoutReachedException(f"Speech recognize process exceeded the timeout of {self._recognize_timeout} seconds.")

            if useUserSetTimeout:
            ...
            if self._diarization == "None":
                ...
                timeout = min(timeout, remaining_recognize_timeout) if useUserSetTimeout else timeout
            else:
                timeout = remaining_recognize_timeout if useUserSetTimeout else 1e9  # Effectively infinite.
            ...
            if useUserSetTimeout and remaining_recognize_timeout <= 0:
                raise UserTimeoutReachedException(f"Speech recognize process exceeded the timeout of {self._recognize_timeout} seconds.")
